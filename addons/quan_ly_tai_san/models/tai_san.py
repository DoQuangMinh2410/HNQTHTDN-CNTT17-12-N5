...
class TaiSan(models.Model):
    ...
    so_luong = fields.Integer('Số lượng tạo', default=1, required=True)
    ...
    @api.depends('gia_tri_ban_dau', 'so_luong')
    def _compute_tong_gia_tri(self):
        for rec in self:
            rec.tong_gia_tri = rec.gia_tri_ban_dau * rec.so_luong
    ...
    # BỔ SUNG TRƯỜNG MỚI THEO CẢI TIẾN
    barcode = fields.Char('Mã vạch (Barcode)', copy=False)
    ngay_het_han_bao_hanh = fields.Date('Ngày hết hạn bảo hành')
    hang_bao_hanh = fields.Char('Hãng bảo hành/NCC')

    # BỔ SUNG LIÊN KẾT KẾ TOÁN
    tk_chi_phi_id = fields.Many2one('tai_khoan_ke_toan', string="Tài khoản Chi phí Khấu hao (Hiệu chuẩn gốc Nợ)")
    tk_hao_mon_id = fields.Many2one('tai_khoan_ke_toan', string="Tài khoản Hao mòn lũy kế (Hiệu chuẩn gốc Có)")
    ...
    def action_tinh_khau_hao(self):
        for record in self:
            ...
            self.env['lich_su_khau_hao'].create({
                ...
                'gia_tri_con_lai': record.gia_tri_hien_tai - so_tien_khau_hao,
                'loai_phieu': 'automatic',
                'ghi_chu': f'Khấu hao tự động {fields.Date.today().strftime("%Y/%m")}'
            })
            
            # Cập nhật giá trị hiện tại
            record.gia_tri_hien_tai -= so_tien_khau_hao
            record.thoi_gian_su_dung += 1

            # EVENT-DRIVEN AUTOMATION: KẾ TOÁN
            if record.tk_chi_phi_id and record.tk_hao_mon_id:
                but_toan = self.env['but_toan_ke_toan'].create({
                    'dien_giai': f'Khấu hao tài sản {record.ma_tai_san} tháng {fields.Date.today().strftime("%m/%Y")}',
                    'tham_chieu': record.ma_tai_san,
                    'nguon_goc': 'he_thong',
                    'chi_tiet_ids': [
                        (0, 0, {
                            'tai_khoan_id': record.tk_chi_phi_id.id,
                            'doi_tuong': record.ma_tai_san,
                            'loai': 'no',
                            'so_tien': so_tien_khau_hao,
                            'dien_giai': 'Ghi chi phí khấu hao TSCĐ'
                        }),
                        (0, 0, {
                            'tai_khoan_id': record.tk_hao_mon_id.id,
                            'doi_tuong': record.ma_tai_san,
                            'loai': 'co',
                            'so_tien': so_tien_khau_hao,
                            'dien_giai': 'Lũy kế hao mòn TSCĐ'
                        })
                    ]
                })
    ...
    def create(self, vals):
        if vals.get('ma_tai_san', 'New') == 'New':
            vals['ma_tai_san'] = self.env['ir.sequence'].next_by_code('tai.san.seq') or 'New'
        
        count = vals.get('so_luong', 1)
        if count > 1:
            # Create first record
            vals_single = vals.copy()
            vals_single['so_luong'] = 1
            first_rec = super(TaiSan, self).create(vals_single)
            
            # Create additional records
            for i in range(count - 1):
                new_vals = vals.copy()
                new_vals['so_luong'] = 1
                new_vals['ma_tai_san'] = self.env['ir.sequence'].next_by_code('tai.san.seq') or 'New'
                super(TaiSan, self).create(new_vals)
